/*
 * server.c v9 — 新增：search --or 多关键词 OR 匹配
 * e.g. search --or 超时 失败
 * 用法: server.exe [管道名]
 */
#include <stdio.h>
...
static void cmd(const char*c){
    ...
    if(strncmp(c,"search ",7)==0){
        ...
        if(use_or)n=idx_search_or(&g_idx,q,r);else n=idx_search_time(&g_idx,q,r,from,to);
