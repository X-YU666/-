/*
 * server.c v9 — 新增：parse_ts 时间解析，search --from/--to 参数
...
/* parse_ts: Unix 时间戳数字 | YYYY-MM-DD | YYYY-MM-DD HH:MM:SS */
...
static void load_persisted(void) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (len == 0) continue;
        LogEntry e;
        if (parse_jsonl_line(line, &e)) { idx_add(&g_idx, &e); loaded++; }
        else errors++;
    }
    fclose(f);
...
static void handle_line(const char *line) {
    LogEntry e;
    if (!parse_jsonl_line(line, &e)) return;
...
static DWORD WINAPI pipe_thread(LPVOID arg) {
    ...
    DisconnectNamedPipe(ctx->pipe); CloseHandle(ctx->pipe); free(ctx);
    return 0;
}
...
static void cmd(const char *c) {
    ...
    if (strcmp(c, "help") == 0) {
        ...
        printf("  search <词> --from <时间>  — 时间下限\n");
        printf("  search <词> --to <时间>    — 时间上限\n");
        printf("    <时间>: Unix 时间戳 | YYYY-MM-DD | YYYY-MM-DD HH:MM:SS\n");
        printf("  list                      — 列出全部\n");
    ...
    if (strcmp(c, "list") == 0) {
        ...
        for (int i = 0; i < g_idx.entry_count; i++) printf("  [%s][%s] %s\n", g_idx.entries[i].node_id, g_idx.entries[i].level, g_idx.entries[i].message);
        return;
    }
    ...
    if (strncmp(c, "search ", 7) == 0) {
        ...
        while (tok) {
            if (strcmp(tok, "--from") == 0 || strcmp(tok, "--to") == 0) {
                int is_from = (tok[2] == 'f');
                tok = strtok(NULL, " ");
                if (tok) {
                    char ts_val[64]; strncpy(ts_val, tok, sizeof(ts_val)-1);
                    tok = strtok(NULL, " ");
                    if (tok && strchr(tok, ':')) {
                    ...
                    if (is_from) from = parse_ts(ts_val); else to = parse_ts(ts_val);
                }
            } else {
        ...
        if (!query[0]) { printf("  用法: search [--from <时间>] [--to <时间>] <关键词>\n"); return; }
        LogEntry r[MAX_ENTRIES];
        int n = idx_search_time(&g_idx, query, r, from, to);
...
static int read_console_line(char *buf, int maxlen) {
    ...
    mode = oldMode; mode &= ~(ENABLE_LINE_INPUT | ENABLE_ECHO_INPUT); mode |= ENABLE_PROCESSED_INPUT;
    ...
    buf[pos] = 0; SetConsoleMode(hIn, oldMode);
    return pos;
}

int main(int argc, char *argv[]) {
    setbuf(stdout, NULL); SetConsoleOutputCP(65001);
    SetConsoleCtrlHandler(signal_handler, TRUE);
    const char *pn = argc > 1 ? argv[1] : "\\\\.\\pipe\\vc_log_agg";
    if (argc > 1 && !validate_pipename(argv[1])) return 1;
    idx_init(&g_idx); buf_init(&g_buf);
    log_info("索引初始化完成");
    g_pf = fopen(PERSIST_PATH, "a");
    if (!g_pf) log_error("无法打开持久化文件 %s", PERSIST_PATH);
    else log_info("持久化文件: %s", PERSIST_PATH);
    load_persisted();
    printf("+------------------------------------------+\n");
    ...
    HANDLE hPipe = CreateNamedPipeA(pn, PIPE_ACCESS_INBOUND, PIPE_TYPE_MESSAGE|PIPE_READMODE_MESSAGE|PIPE_WAIT,
    ...
    while (g_running) {
        ...
        if (wr == WAIT_OBJECT_0) {
            log_info("Agent 连接");
            ClientCtx *ctx = malloc(sizeof(ClientCtx)); ctx->pipe = hPipe;
    ...
    if (g_pf) fclose(g_pf);
    log_info("关闭完毕: 交付 %d 条, 残留 %d 条, 错误 %d 次", g_idx.entry_count, g_buf.buf_count, g_error_count);
